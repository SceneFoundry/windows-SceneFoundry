...
void _curve_to(VkvgContext ctx, float x1, float y1, float x2, float y2, float x3, float y3) {
    // _add_point stores device-space points, but subdivision and its control
    // points are in user space. Undo mat1 here so subdivision applies it once.
    vec2 cp = {x1, y1};
    if (!_current_path_is_empty(ctx)) {
        cp = _get_current_position(ctx);
        vkvg_matrix_t inverse = ctx->mat1;
        if (vkvg_matrix_invert(&inverse) != VKVG_STATUS_SUCCESS) {
            ctx->status = VKVG_STATUS_INVALID_MATRIX;
            return;
        }
        vkvg_matrix_transform_point(&inverse, &cp.x, &cp.y);
    }

    // prevent running _recursive_bezier when all 4 curve points are equal
    if (EQUF(x1, x2) && EQUF(x2, x3) && EQUF(y1, y2) && EQUF(y2, y3)) {
        if (_current_path_is_empty(ctx) || (EQUF(cp.x, x1) && EQUF(cp.y, y1)))
            return;
    }
    ctx->simpleConvex = false;
    _set_curve_start(ctx);
...
void vkvg_restore(VkvgContext ctx) {
    ...
    if (sav->pattern) {
        if (sav->pattern != ctx->pattern) {
            ...
            /* _update_cur_pattern adopts the pointer without adding a
             * reference. Transfer the saved-state reference to the context
             * so _free_ctx_save does not release it a second time. */
            sav->pattern = NULL;
        }
    } else {
...
void vkvg_ellipse(VkvgContext ctx, float radiusX, float radiusY, float x, float y, float rotationAngle) {
    ...
    // Eight 45-degree cubics: k = 4/3 * tan(pi/16). Two semicircle
    // cubics bulge by 1.835% of the radius, visibly rotating against raster rings.
    // Fixed endpoints make the final endpoint exactly equal to the first.
    static const float unit[9][2] = {
        {0.0f, -1.0f}, {0.7071067811865475f, -0.7071067811865475f},
        {1.0f, 0.0f}, {0.7071067811865475f, 0.7071067811865475f},
        {0.0f, 1.0f}, {-0.7071067811865475f, 0.7071067811865475f},
        {-1.0f, 0.0f}, {-0.7071067811865475f, -0.7071067811865475f},
        {0.0f, -1.0f}
    };
    const float k = 0.2652164898395440f;
    const float c = cosf(rotationAngle);
    const float s = sinf(rotationAngle);

    _finish_path(ctx);
    _add_point(ctx, x + s * radiusY, y - c * radiusY);
    for (int i = 0; i < 8; ++i) {
        const float u0 = unit[i][0], v0 = unit[i][1];
        const float u1 = unit[i + 1][0], v1 = unit[i + 1][1];
        // Tangent on the unit circle is (-v, u); scale it with the ellipse.
        const float x1 = radiusX * (u0 - k * v0);
        const float y1 = radiusY * (v0 + k * u0);
        const float x2 = radiusX * (u1 + k * v1);
        const float y2 = radiusY * (v1 - k * u1);
        const float x3 = radiusX * u1, y3 = radiusY * v1;
        _curve_to(ctx, x + c * x1 - s * y1, y + s * x1 + c * y1,
                       x + c * x2 - s * y2, y + s * x2 + c * y2,
                       x + c * x3 - s * y3, y + s * x3 + c * y3);
        if (vkvg_status(ctx))
            return;
    }
